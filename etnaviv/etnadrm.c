...
int viv_open(enum viv_hw_type hw_type, struct viv_conn **out)
{
	...
	sv.drm_dd_major = -1;
	sv.drm_dd_minor = -1;
	if (drmSetInterfaceVersion(conn->fd, &sv))
		goto error;
...
//fprintf(stderr, "cmd: type %u submit_idx %u submit_offset 0x%x size 0x%x relocs 0x%llx nr_relocs %u\n",
//	cmd.type, cmd.submit_idx, cmd.submit_offset, cmd.size, cmd.relocs, cmd.nr_relocs);
...
//fprintf(stderr, "req: pipe %u cmd 0x%llx nr_cmds %u bos 0x%llx nr_bos %u\n",
//	req.pipe, req.cmds, req.nr_cmds, req.bos, req.nr_bos);
